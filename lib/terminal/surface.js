...
Surface.computedProperty(function width(new_width, old_width) {
	this.lines = this._createLinesBuffer(this.computed.height);
	return new_width || 0;
});
...
Surface.computedProperty(function height(new_height, old_height) {
	this.lines = this._createLinesBuffer(new_height || 0);
	return new_height || 0;
});
 ...
 * @type     {Array}
 */
Surface.computedProperty(function lines(new_lines, old_lines) {

	if (new_lines == null) {
		new_lines = this.createLinesBuffer();
	}
...
Surface.setMethod(function createLinesBuffer() {
	return this._createLinesBuffer(this.computed.height);
});
 ...
 * @version  0.1.0
 */
Surface.setMethod(function _createLinesBuffer(height) {
...
Surface.setMethod(function getCell(x, y) {
	return this.computed.lines[y].getCell(x);
});
