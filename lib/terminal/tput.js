 ...
 */
Tput = Fn.inherits('Informer', 'Vexation', function Tput(vexation) {
...
Tput.setMethod(function parse(terminfo_path) {
	...
	headers = {
		...
		header_size       : 12,
		magic_number      : (file[ 1] << 8) | file[ 0],
		names_size        : (file[ 3] << 8) | file[ 2],
		bool_count        : (file[ 5] << 8) | file[ 4],
		number_count      : (file[ 7] << 8) | file[ 6],
		string_count      : (file[ 9] << 8) | file[ 8],
		string_table_size : (file[11] << 8) | file[10]
	};
	...
	max = index + headers.number_count * 2;
...
Tput.setMethod(function get(name) {
	...
	if (!this.informations) {
		throw new Exception('No terminfo has been parsed yet');
	}
